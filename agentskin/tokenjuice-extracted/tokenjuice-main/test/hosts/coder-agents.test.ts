...
describe("Coder Agents skill", () => {
  ...
  it("constrains explicit skill paths to the project boundary", async () => {
    ...
    process.chdir(home);
    await expect(installCoderAgentsSkill(outsideSkillPath)).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
    await expect(installCoderAgentsSkill(outsideSkillPath, { projectDir: home })).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
    await expect(uninstallCoderAgentsSkill(outsideSkillPath, { projectDir: home })).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
  ...
  it("rejects explicit skill paths under symlinked parents inside or outside projectDir", async () => {
    ...
    ).rejects.toThrow(/cannot use .+; tokenjuice only installs the project-local/u);
    await expect(
