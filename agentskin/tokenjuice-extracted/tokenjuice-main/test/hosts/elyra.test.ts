...
describe("elyra skill", () => {
  ...
  it("constrains explicit skill paths to the project boundary", async () => {
    ...
    process.chdir(home);
    await expect(installElyraSkill(outsideSkillPath)).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
    await expect(installElyraSkill(outsideSkillPath, { projectDir: home })).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
    await expect(uninstallElyraSkill(outsideSkillPath, { projectDir: home })).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
