...
describe("mcp-agent definition", () => {
  ...
  it("constrains explicit agent paths to the project boundary", async () => {
    ...
    process.chdir(home);
    await expect(installMcpAgentDefinition(outsideAgentPath)).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
    await expect(installMcpAgentDefinition(outsideAgentPath, { projectDir: home })).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
    await expect(uninstallMcpAgentDefinition(outsideAgentPath, { projectDir: home })).rejects.toThrow(/cannot use .+; tokenjuice will not/u);

    const doctor = await doctorMcpAgentDefinition(outsideAgentPath, { projectDir: home });

    expect(doctor.status).toBe("broken");
    expect(doctor.hasTokenjuiceMarker).toBe(false);
    expect(doctor.issues[0]).toContain("cannot use");
  ...
  it("rejects explicit agent paths under symlinked parents outside projectDir", async () => {
    ...
    await expect(installMcpAgentDefinition(linkedAgentPath, { projectDir: home })).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
