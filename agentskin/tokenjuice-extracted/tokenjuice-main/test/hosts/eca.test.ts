...
describe("eca skill", () => {
  ...
  it("constrains explicit skill paths to the project boundary", async () => {
    ...
    process.chdir(home);
    await expect(installEcaSkill(outsideSkillPath)).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
    await expect(installEcaSkill(outsideSkillPath, { projectDir: home })).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
    await expect(uninstallEcaSkill(outsideSkillPath, { projectDir: home })).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
