...
describe("Greptile rules", () => {
  ...
  it("constrains explicit rule paths to the project boundary", async () => {
    ...
    process.chdir(home);
    await expect(installGreptileRule(outsideRulePath)).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
    await expect(installGreptileRule(outsideRulePath, { projectDir: home })).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
    await expect(uninstallGreptileRule(outsideRulePath, { projectDir: home })).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
