...
describe("pi-go skill", () => {
  ...
  it("constrains explicit skill paths to the project boundary", async () => {
    ...
    process.chdir(home);
    await expect(installPiGoSkill(outsideSkillPath)).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
    await expect(installPiGoSkill(outsideSkillPath, { projectDir: home })).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
    await expect(uninstallPiGoSkill(outsideSkillPath, { projectDir: home })).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
