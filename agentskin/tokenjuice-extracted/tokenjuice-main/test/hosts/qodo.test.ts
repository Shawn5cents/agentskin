...
describe("Qodo review config", () => {
  ...
  it("constrains explicit config paths to the project boundary", async () => {
    ...
    process.chdir(home);
    await expect(installQodoReviewConfig(outsideConfigPath)).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
    await expect(installQodoReviewConfig(outsideConfigPath, { projectDir: home })).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
    await expect(uninstallQodoReviewConfig(outsideConfigPath, { projectDir: home })).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
