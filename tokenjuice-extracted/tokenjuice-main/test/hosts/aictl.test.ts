...
describe("aictl instructions", () => {
  ...
  it("constrains explicit instruction paths to the project boundary", async () => {
    ...
    process.chdir(home);
    await expect(installAictlInstructions(outsideInstructionsPath)).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
    await expect(installAictlInstructions(outsideInstructionsPath, { projectDir: home })).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
    await expect(uninstallAictlInstructions(outsideInstructionsPath, { projectDir: home })).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
  ...
  it("rejects explicit instruction paths under symlinked parents inside or outside projectDir", async () => {
    ...
    await expect(installAictlInstructions(join(linkedOutsideDir, "AICTL.md"), { projectDir: home })).rejects.toThrow(
      /cannot use .+; tokenjuice only installs the project-local/u,
    );
