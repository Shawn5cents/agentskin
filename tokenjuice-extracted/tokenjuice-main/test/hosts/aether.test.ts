...
describe("Aether prompt integration", () => {
  ...
  it("constrains explicit prompt paths to the project boundary", async () => {
    ...
    process.chdir(home);
    await expect(installAetherPrompt(outsidePromptPath)).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
    await expect(installAetherPrompt(outsidePromptPath, { projectDir: home })).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
    await expect(uninstallAetherPrompt(outsidePromptPath, { projectDir: home })).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
  ...
  it("rejects explicit prompt paths under symlinked parents outside projectDir", async () => {
    ...
    await expect(installAetherPrompt(linkedPromptPath, { projectDir: home })).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
