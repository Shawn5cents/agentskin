...
describe("Baz skill", () => {
  ...
  it("constrains explicit skill paths to the project boundary", async () => {
    ...
    process.chdir(home);
    await expect(installBazSkill(outsideSkillPath)).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
    await expect(installBazSkill(outsideSkillPath, { projectDir: home })).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
    await expect(uninstallBazSkill(outsideSkillPath, { projectDir: home })).rejects.toThrow(/cannot use .+; tokenjuice will not/u);
