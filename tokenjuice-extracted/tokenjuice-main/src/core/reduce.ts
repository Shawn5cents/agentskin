...
import { applyJsonSemantic } from "./json-semantic.js";
...
export async function reduceExecutionWithRules(
  ...
  // JSON semantic dispatch: if the matched rule has jsonSemantic config,
  // try to parse the output as JSON and apply semantic pruning.
  if (matchedRule.rule.jsonSemantic) {
    const trimmed = rawText.trim();
    if (trimmed.startsWith("{") || trimmed.startsWith("[")) {
      try {
        const parsed = JSON.parse(trimmed) as unknown;
        const jsConfig = matchedRule.rule.jsonSemantic;
        const urlArg = reducerInput.argv?.find((arg: string) => arg.startsWith("http")) ?? "";
        const url = jsConfig.urlMatches ?? urlArg;
        const result = applyJsonSemantic(parsed, {
          url,
          ...(jsConfig.signals !== undefined ? { signals: jsConfig.signals } : {}),
          ...(jsConfig.aliases !== undefined ? { aliases: jsConfig.aliases } : {}),
          ...(jsConfig.smallThreshold !== undefined ? { smallThreshold: jsConfig.smallThreshold } : {}),
          ...(jsConfig.stripAnsi !== undefined ? { stripAnsiCodes: jsConfig.stripAnsi } : {}),
        });
        if (result.metrics.applied && result.skin) {
          const exitPrefix = reducerInput.exitCode && reducerInput.exitCode !== 0
            ? `exit ${reducerInput.exitCode}\n`
            : "";
          const inlineText = `${exitPrefix}${result.skin}`;
          const reducedChars = countTextChars(inlineText);
          const semanticClassification = {
            family: matchedRule.rule.family,
            confidence: classification.confidence,
            matchedReducer: matchedRule.rule.id,
            ...(classification.matchedVia
              ? { matchedVia: classification.matchedVia }
              : {}),
            ...(classification.matchedCommand
              ? { matchedCommand: classification.matchedCommand }
              : {}),
          };
          ...
          return {
            ...
            ...(trace
              ? {
                  trace: {
                    ...trace,
                    matchedReducer: semanticClassification.matchedReducer,
                    family: semanticClassification.family,
                  },
                }
              : {}),
            ...(rawRef ? { rawRef } : {}),
        ...
        // JSON parse failed — fall through to normal rule processing
      }
    }
  }
