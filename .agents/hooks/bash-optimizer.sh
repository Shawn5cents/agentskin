...
... [truncated ${#stdout_str} bytes -> ${TOKENJUICE_MAX_RAW}] ..."
  ...
  # NOTE: tmp_out/tmp_err are file paths — node reads from disk directly
  # to avoid passing multi-MB content as argv (which hits ARG_MAX on Linux).
  local payload
  payload=$(node -e '
    const fs = require("fs");
    const args = JSON.parse(process.argv[1] || "[]");
    const out = fs.readFileSync(process.argv[2], "utf8");
    const err = fs.readFileSync(process.argv[3], "utf8");
  ...
  ' "${argv_json}" "${tmp_out}" "${tmp_err}" "${exit_code}" "${duration_ms}" "${TOKENJUICE_TOOL}" "${PWD}" 2>/dev/null)
